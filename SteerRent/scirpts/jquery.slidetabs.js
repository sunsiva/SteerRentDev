...
(function($)
	...
	function SlideTabs(c,d)
		...
		var e=this,href,fragmentId=/^#.+/,hrefBase,baseEl,slug;
		...
		var h=g(navigator.userAgent),browser=
			{
		};
		if(h.browser)
			{
		...
		if(d.orientation=='horizontal')
			...
			var j=this.$prev.outerHeight(true),nextBtnH=this.$next.outerHeight(true);
	...
	SlideTabs.prototype=
		...
		,resize_height:function()
			{
			var a=(this.$contentCont.outerHeight(true)-this.$contentCont.height()),newContentHeight;
		...
		,tabs_setAutoWidth:function()
			{
			this.tabs_setSlideLength();
			var a=this.tabs_getTotalLength(),tabsContW=(this.buttonsVisible)?parseInt(this.$tabsInnerCont.css('width')):parseInt(this.$tabsCont.css('width'));
			if(this.$container.hasClass(this.conf.classTabSlidingEnabled))
				{
		...
		,tabs_setAutoHeight:function()
			...
			this.resizeTimer=setTimeout(function()
				...
				if(b.$tabs.outerHeight(false)<b.$tabsCont.outerHeight(true))
					...
					{
					var a,$liLast=b.$lis.last(),$unalignedLi,gap=b.val.tabsSlideLength-($liLast.position().top+$liLast.outerHeight(false)),alignedTop=false,alignedBottom=false;
		...
		,tabs_load:function(a,b,c,d)
			...
			this.xhr=$.ajax(
				{
				url:b,dataType:'html',success:function(r)
					{
		...
		,tabs_slidePrev:function(b)
			...
			var c=this,$lis=this.$tabs.children('li');
			$lis.each(function()
				{
				c.$li=$(this);
				...
				if(c.val.elemP>=(-1+c.conf.offsetTL))
					...
					if(c.conf.tabsToSlide>1)
						{
						var a=c.$li.index(),index=((a-c.conf.tabsToSlide)),isFirst=(a>0)?1:0;
		...
		,tabs_slideNext:function(a)
			...
			var b=this,$lis=this.$tabs.children('li'),$thisA;
			$lis.each(function()
				{
				b.$li=$(this);
		...
		,tabs_animate:function(a)
			{
			var b=this,animSpeed=(a>0)?a:b.conf.tabsAnimSpeed;
			if(b.val.useWebKit)
				{
				b.tabs_bindWebKitCallback();
				b.$tabs.css(
					{
					'-webkit-transition-duration':animSpeed+'ms','-webkit-transition-timing-function':'ease-out','-webkit-transform':b.val.pre+ -b.margin+b.val.px
				}
				)
			}
			else
		...
		,tabs_WebKitPosition:function(a,b)
			{
			var c=window.getComputedStyle(a.get(0),null).getPropertyValue('-webkit-transform'),wkValArray=c.replace(/^matrix\(/i,'').split(/, |\)$/g),val=parseInt(wkValArray[b],10);
		...
		,content_setParentsHeight:function(a)
			{
			var b=this,$this,$content,$viewInner,total=b.$parentViews.length,isLast,height;
		...
		,content_webKitSlide:function(a,b)
			...
			this.$viewActive.css(
				{
				'-webkit-transition-duration':a+'ms','-webkit-transition-timing-function':b,'-webkit-transform':this.content.pre+this.val.animVal+this.content.px
			}
			);
			this.$view.css(
				{
				'-webkit-transition-duration':a+'ms','-webkit-transition-timing-function':b,'-webkit-transform':'translate3d(0px,0px,0px)'
			}
			)
		}
		...
		,content_slideH:function(a)
			...
			if(b.val.useWebKit)
				...
				if(a>0)
					...
					b.$view.css(
						{
						'-webkit-transition-duration':'0ms','-webkit-transform':'translate3d('+b.val.cssVal+'px,0px,0px)'
					}
					);
					setTimeout(function()
						{
		...
		,content_slideV:function(a)
			...
			if(b.val.useWebKit)
				...
				if(a>0)
					...
					b.$view.css(
						{
						'-webkit-transition-duration':'0ms','-webkit-transform':'translate3d(0px,'+b.val.cssVal+'px,0px)'
					}
					);
					setTimeout(function()
						{
		...
		,addTab:function(a,b,c)
			...
			this.$tabs.append('<li><a href="#'+d.slug+'" rel="'+d.slug+'" class="'+this.conf.classTab+' st_tab_'+d.total+'">'+a+'</a><button class="cancel_btn">X</button></li>');
	...
	$.fn.slidetabs=function(a)
		{
		var b,conf=$.extend(true,
			{
		}
		,$.fn.slidetabs.defaults,a),returnArr=[];
		this.each(function()
			{
			b=this;
			if(!b.slidetabs)
